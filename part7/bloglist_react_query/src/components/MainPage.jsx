...
export default function MainPage() {
    ...
    const specificUser = userMatch && userList.data
        ? userList.data.find(user => user.id === String(userMatch.params.id))
        : null
    ...
    const specificBlog = blogMatch && blogResult.data
        ? blogResult.data.find(blog => blog.id === String(blogMatch.params.id))
        : null

    useEffect(() => {
        console.log('User data', userList.data)
        console.log('Blog data', blogResult.data)
        // setUsers(userList.data)
    }, [userList.data, blogResult.data])


    const newBlogMutation = useMutation({
        mutationFn: blogService.createNewNote,
        onSuccess: (response) => {
            queryClient.invalidateQueries({queryKey: [['blogs'], ['userList']]});
    ...
    return (
        <div>
            ...
            <Link to={'/blogs'} style={{margin: '8px'}}>blogs</Link>
            <Link to={'/users'} style={{margin: '8px'}}>users</Link>
            ...
            <Routes>
                <Route path='/' element={renderMainPage(blogs)} />
                <Route path='/blogs' element={renderMainPage(blogs)} />
                <Route path='/blogs/:id' element={<BlogPage blog={specificBlog}/> }/>
                <Route path='/users' element={<UsersPage users={userList.data} />} />
